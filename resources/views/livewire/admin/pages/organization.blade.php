<div>
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
            <x-table title="Data Organization" :heads="$table_heads">
                @forelse ($organizations as $key => $organization)
                    <tr wire:key="user-{{ $organization->id }}"
    ...
    <x-modal name="organization-modal" :show="$errors->isNotEmpty()" maxWidth="md" focusable>
        <form wire:submit="save" class="p-6">
            <!-- Header with close button -->
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                    {{ $editId ? __('Edit Organization') : __('Create Organization') }}
                </h2>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input wire:model="form.name" id="name" name="name" type="text"
                        class="block w-full mt-1" placeholder="{{ __('e.g. Wilayah D, OMK D') }}" />
                    <x-input-error :messages="$errors->get('form.name')" class="mt-2" />
                </div>

                <!-- Code Field -->
                <div>
                    <x-input-label for="code" value="{{ __('Code') }}" class="mb-1" />
                    <x-text-input wire:model="form.code" id="code" name="code" type="text"
                        class="block w-full mt-1" placeholder="{{ __('e.g. WD, OMK') }}" />
    ...
    <x-modal name="delete-organization-confirmation" :show="$errors->isNotEmpty()" focusable>
        <form wire:submit="delete" class="p-6">
            ...
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Please confirm that you want to delete this organization by clicking the button below.') }}
            </p>
