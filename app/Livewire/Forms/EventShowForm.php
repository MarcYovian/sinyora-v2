...
class EventShowForm extends Form
{
    public ?Event $event;
        ...
        if ($event) {
            ...
            if ($event->eventRecurrences()->exists()) {
                ...
                foreach ($dates as $key => $date) {
                    ...
                    $isConsecutiveDay = $prevDate->addDay()->equalTo($currentDate);
                    $startsAtMidnight = Carbon::parse($date->time_start)->format('H:i:s') === '00:00:00';
                    $prevEndsAtMidnight = Carbon::parse($dates[$key - 1]->time_end)->format('H:i:s') === '23:59:59';
                    // Jika tanggal saat ini adalah besok dari tanggal sebelumnya
                    if ($isConsecutiveDay && $startsAtMidnight && $prevEndsAtMidnight) {
                        $endDate = $date;
                    } else {
                        break;
                    }
                }
                // dd($startDate->toArray(), $endDate->toArray());

                // Format datetime
                $this->start_datetime = Carbon::parse($startDate->date->format('Y-m-d') . ' ' . $startDate->time_start->format('H:i:s'))
                    ->format('Y-m-d\TH:i');
                $this->end_datetime = Carbon::parse($endDate->date->format('Y-m-d') . ' ' . $endDate->time_end->format('H:i:s'))
                    ->format('Y-m-d\TH:i');
            }
        }
    }
